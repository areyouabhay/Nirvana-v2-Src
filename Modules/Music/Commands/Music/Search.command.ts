  ...
  public async run(client: Bot, ctx: Context, args: string[]): Promise<any> {
    ...
    if (!res) {
      let embed = this.client
        ...
        .setDescription(
          `**${ctx.author.tag}**, No results found for your search: **${query}**`
        );
        ...
        ctx.sendMessage({
          embeds: [
            embed
              .setColor(this.client.color.red)
              .setDescription(
                `**${ctx.author.tag}**, An **Error** arised while searching for your Query!`
              ),
          ],
        });
        break;
        ...
        ctx.sendMessage({
          embeds: [
            embed
              .setColor(this.client.color.red)
              .setDescription(
                `**${ctx.author.tag}**,  No search results were found for your query: **${query}**`
              ),
          ],
        });
        break;
        ...
        const SearchResults = tracks.map(
          (track: Song, index: number) =>
            `\`[${index + 1}]\` | [${track.info.title}](${
              track.info.uri
            }) (\`${this.client.botfunctions.formatTime(track.info.length)}\`)`
        );
      ...
      await ctx.editMessage({
        embeds: [
          embed
            .setTitle(null)
            .setDescription(
              `Added [${song.info.title}](${this.client.config.url.support}) by [${song.info.author}](${this.client.config.url.support}) to the queue`
            ),
        ],
        components: [
          {
